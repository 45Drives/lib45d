...
void test_construction(void) {
	...
	std::cout
		<< "Testing construction from \"50%\" == from 0.5 == from max/2 == from bytes string == "
		<< a.get() / 2 << std::endl;
	ffd::Quota str_quota(a, "50%");
	ffd::Quota double_quota(a, 0.5);
	ffd::Quota bytes_quota(a, a / 2);
	ffd::Quota bytes_str_quota(a, (a / 2).get_str());
	assert(str_quota == double_quota && double_quota == bytes_quota
		   && bytes_quota == bytes_str_quota && bytes_str_quota == a.get() / 2);
...
void test_string(void) {
	...
	std::cout << "asserting 50% of " << bytes.get_str(ffd::Bytes::PrefixType::SI, 0) << " is "
			  << (bytes / 2).get_str(ffd::Bytes::PrefixType::SI, 0) << std::endl;
	assert(quota.get_str(ffd::Bytes::PrefixType::SI, 0)
		   == (bytes / 2).get_str(ffd::Bytes::PrefixType::SI, 0));
	std::cout << "OK" << std::endl;
}
